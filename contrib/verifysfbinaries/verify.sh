...
###   This script attempts to download the signature file SHA256SUMS.asc from aurumcoin.org
...
WORKINGDIR="/tmp/aurumcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://aurumcoin.org/bin/"
VERSIONPREFIX="aurumcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'aurumcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Aurumcoin Core binary release signing key installed?"
   fi
